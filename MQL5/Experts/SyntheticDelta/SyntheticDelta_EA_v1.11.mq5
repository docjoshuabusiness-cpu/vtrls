...
//+------------------------------------------------------------------+
//| COSTANTI STATISTICHE TESTER (enum ENUM_STATISTICS di MQL5)       |
//+------------------------------------------------------------------+
#define STAT_PROFIT               1   // Profitto netto totale
#define STAT_BALANCE_MAX_DRAWDOWN 7   // Max drawdown assoluto sul balance
#define STAT_TRADES               20  // Numero totale operazioni chiuse
#define STAT_TRADES_WIN           21  // Numero operazioni chiuse in profitto
...
{
   Print("============================================================");
   Print("SyntheticDelta EA v1.11 - Standalone + Volatility Regime + Expansion Candle Filter");
   Print("============================================================");
...
//+------------------------------------------------------------------+
//| CalcLotSize                                                       |
//| ATTENZIONE: il rischio monetario è sempre calcolato sulla base di |
//| Inp_StopLoss (punti fissi), ANCHE quando Inp_UseATR_SLTP=true e   |
//| lo SL reale in esecuzione è un multiplo di ATR. Se i due valori   |
//| divergono, il rischio effettivo per trade differisce da           |
//| Inp_RiskPct target. Da correggere se si vuole risk sizing         |
//| coerente con SL dinamico ATR.                                     |
//+------------------------------------------------------------------+
double CalcLotSize()
{
   ...
   if(tickValue <= 0.0 || tickSize <= 0.0 || point <= 0.0) return Inp_LotSize;

   double slTicks = (Inp_StopLoss * point) / tickSize;
...
{
   Print("============================================================");
   Print("STATISTICHE PER ORA - SyntheticDelta EA v1.11");
   Print("============================================================");
...
double OnTester()
{
   ...
   double max_dd        = TesterStatistics(STAT_BALANCE_MAX_DRAWDOWN);
   double win_trades    = TesterStatistics(STAT_TRADES_WIN);
